...
public class Jeu {
	...
	public void partie() {
		...
		ArrayList<Personnage> ia = new ArrayList<Personnage>();
		
		for (Personnage p: this.participants ) {
			if (p.isJoueur()) {joueurs.add(p);}
			else {ia.add(p);}
		}
		
		boolean iaVie = false;
		boolean joueursVie = false;
		...
		while (continuer) {
			...
			for (Personnage p:joueurs) {
				joueursVie= joueursVie || (p.isJoueur() && p.getHp()>0);
			}
			
			for (Personnage p:ia) {
				iaVie= iaVie || (!(p.isJoueur()) && p.getHp()>0);
			}
			
			continuer = iaVie && joueursVie; 
